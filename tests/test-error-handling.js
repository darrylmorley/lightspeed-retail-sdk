...
async function testErrorHandling() {
  ...
  console.log(`   ✅ Result: ${items.length} items (${Array.isArray(items) ? 'Array' : typeof items})`);

  console.log("   Testing getCustomers() with invalid credentials:");
  const customers = await invalidSDK.getCustomers({ limit: 5 });
  console.log(`   ✅ Result: ${customers.length} customers (${Array.isArray(customers) ? 'Array' : typeof customers})`);

  console.log("   Testing getSales() with invalid credentials:");
  const sales = await invalidSDK.getSales({ limit: 5 });
  console.log(`   ✅ Result: ${sales.length} sales (${Array.isArray(sales) ? 'Array' : typeof sales})`);
  ...
  console.log(`   ✅ Result: ${categories.length} categories (${Array.isArray(categories) ? 'Array' : typeof categories})`);

  console.log("   Testing getVendors() with invalid account ID:");
  const vendors = await invalidAccountSDK.getVendors({ limit: 5 });
  console.log(`   ✅ Result: ${vendors.length} vendors (${Array.isArray(vendors) ? 'Array' : typeof vendors})`);
  ...
  console.log(`   ✅ Result: ${employees.length} employees (${Array.isArray(employees) ? 'Array' : typeof employees})`);
  ...
  try {
    ...
    console.log(`   ✅ Result: ${Array.isArray(item) ? item.length + ' items' : typeof item}`);
  } catch (error) {
  ...
  try {
    ...
    console.log(`   ✅ Result: ${Array.isArray(customer) ? customer.length + ' customers' : typeof customer}`);
  } catch (error) {
    console.log(`   ✅ Properly caught parameter error: ${error.message}`);
  }
  ...
  console.log("   Testing getItems() with very recent timestamp (should be empty):");
  const recentItems = await emptyTestSDK.getItems({ timeStamp: veryRecentTime, limit: 5 });
  console.log(`   ✅ Result: ${recentItems.length} items (${Array.isArray(recentItems) ? 'Array' : typeof recentItems})`);

  console.log("   Testing getOrders() with very recent timestamp (should be empty):");
  const recentOrders = await emptyTestSDK.getOrders({ timeStamp: veryRecentTime, limit: 5 });
  console.log(`   ✅ Result: ${recentOrders.length} orders (${Array.isArray(recentOrders) ? 'Array' : typeof recentOrders})`);

  // Summary
  console.log("\n📊 Error Handling Test Summary:");
  console.log("✅ All getter methods return empty arrays instead of throwing errors");
  console.log("✅ No verbose error logging for common failure scenarios");
  console.log("✅ Consistent return types (always arrays for collection methods)");
